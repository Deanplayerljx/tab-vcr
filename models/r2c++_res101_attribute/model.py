...
@Model.register("MultiHopAttentionQABUA")
class MultiHopAttentionQABUA(Model):
    def __init__(self,
                 vocab: Vocabulary,
    ...
    def forward(self,
        ...
        q_rep, q_obj_reps = self.embed_span(question, question_tags, question_mask, obj_reps)
        a_rep, a_obj_reps = self.embed_span(answers, answer_tags, answer_mask, obj_reps)
        ...
        atoo_similarity = self.obj_attention(a_rep.view(a_rep.shape[0], a_rep.shape[1] * a_rep.shape[2], -1),
                                             obj_reps).view(a_rep.shape[0], a_rep.shape[1],
                                                            a_rep.shape[2], obj_reps.shape[1])
        atoo_attention_weights = masked_softmax(atoo_similarity, box_mask[:,None,None])
        attended_o = torch.einsum('bnao,bod->bnad', (atoo_attention_weights, obj_reps))
